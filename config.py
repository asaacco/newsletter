...
6. title에 '[외신]' 이 있는 경우, 한국어 문장으로 번역하고, 고유명사는 영문으로 표기.
7. 각 카테고리 별 (예: "ESG_소식", "AI_소식") 4개 이상의 중요 기사를 선정.
"""

OBJECTIVE = """
1. 카테고리별 독립 처리: 입력받은 JSON에 여러 최상위 카테고리(예: "ESG_소식", "AI_소식")가 있으면 카테고리마다 따로 처리합니다.
2. 중복 기사 식별: 동일·유사 기사(80% 이상 유사)는 대표 기사만 남기고, 나머지 1개만 선택하여 "additional_source" 에 추가합니다.
...
5. 3 문장 요약: "new_summary_with_insight" 필드에 3문장으로 작성하고, 제목과 중복되지 않도록 합니다.
6. 자연스러운 한국어: 번역체를 피하고, ~니다. 형태를 사용하며, 차별·논란 표현은 완화합니다.
7. 핵심 정보·수치 포함: 요약문에 최대한 핵심 정보나 수치를 담습니다.
8. 인공지능 말투 금지: "이는...", "이처럼..." 등의 표현 사용 금지.
...
- title이 '[외신]'으로 시작하는 기사의 경우 최우선적으로 처리하고 결과에 포함하세요.
