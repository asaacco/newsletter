...
def generate_section(news_list):
    ...
    # 뉴스 항목을 HTML로 변환
    section_html = "<div class='news-section'><br>"
    print(news_list)

    for article in news_list:
        section_html += f'''
        <div class="news-item">
            <p style="color: #003366; font-size: 12px; line-height: 1.2;"><strong><a href="{article["link"]}">{article["title"]}</a></strong></p> 
            <p style="color: #003366; font-size: 12px; line-height: 1.2;">[{article["media_name"]}] ({article["date"]})</p>
            <p style="color: #003366; font-size: 10px; line-height: 1.5;">{article["new_summary_with_insight"]}</p><br>
        '''
            # <p>* 해당 기사: <a href="{article["link"]}">{article["link"]}</a></p>
        # 키 존재 여부를 확인하여 안전하게 처리
        if article.get("additional_source"):
            section_html += f'<p>* 다른 기사: <a href="{article["additional_source"]}">{article["additional_source"]}</a></p>'
        section_html += '</div><br>'
    section_html += '</div>'
    return section_html
...
def json_to_string(selected_domain, json_file_path: str) -> str:
    ...
    lines.append(f"* 출처: https://asaac.co.kr\n")
    lines.append(f"* 더 자세한 {title} 관련 뉴스 기사 및 인사이트\n")
    lines.append(f"* https://open.kakao.com/o/grtDi31g\n")
    lines.append(f"Copyright {TODAY_YEAR}. ASAAC Co. all rights reserved.\n")
